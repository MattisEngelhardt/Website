...
|---|---|---|---|
| A · Main-Lobby (Nebelmeer-Hub) | ✅ | **19.06. AUF `/` INTEGRIERT (A5/A6) + Look-Pass + Mattis-Verriss #2 gefixt.** (17.06.: fog.glb + carved frames + Figur weg.) **19.06.:** (1) **Einheitliche painterly Stills** (`scripts/lib/plate-grade.mjs` + `grade-plates.mjs`): Sepia-Unify + Lift + Warm-Wash → alle 4 Plates EINE Golden-Hour-Welt (Camino-Grün/City-Neon weg). (2) **Nebel leuchtet** (uFogTint hart auf Warmweiß, Kontrast 1.7→1.4) = hellste Masse; **Aivazovsky-Himmelsbande** + Sonnen-Scatter. (3) **ForeRock** (`plinth_set.py`→`plinths.glb`) ersetzt crude Outcrop = dunkler Repoussoir-Saum (Plinth-Ledges deferred — schwebten). (4) **Peaks** (`peaks_set.py`→`peaks.glb`, 5 Ridges ~19k tris) = echte Friedrich-Bergketten hinter dem Nebel → „mit Blender gemacht". (5) **Blöcke entzerrt**: PLATE_DEFS über weiten Azimut-Bogen (−36…+30°) + Tiefen/Höhen-Versatz → KEINE Überlappung mehr (Mattis-Kritik #2). (6) **Integration**: `/` pusht durch Friedrichs echtes Gemälde (threshold.ts) → dissolvt in den Hub (worlds/summit.ts mountHub, index.astro `#hub-canvas`+Placard-`<a>`), **alte Figur weg**. check 0 / build grün (14 S.) / Hub 56-57fps headless / homepage 4 Placards 0 Errors. | 19.06. |
| B · My Vacations (Galerie + Pipeline) | ✅ | **19.06.:** 3 parallele Opus-Agenten (Pipeline + Galerie-Welt). `/vacations` = gekrümmte phantom.land-Drag-Wall in Goldrahmen + Lightbox. Pipeline (`make-vacation-media.mjs`+`make-vacation-video.mjs`, sharp+ffmpeg via **ffmpeg-static** devDep): 19 Fotos AVIF/WebP-srcset+LQIP, 16 Videos <1MB-Preview-Loops + 1080p-Lightbox (`-t 30 maxrate 3.5M` < 25MB) + Poster, Montenegro auf 10 kuratiert, Manifest 35 Items. Welt: GPU-DOM-Wall, Drag+Momentum, IO-Virtualisierung (≤5 Videos), Hover-Preview, Lightbox (Ton/Pfeile/ESC/Focus-Trap), reduced-motion-Fallback, statische SEO-Galerie. **2 Bugs gefixt:** pointer-capture stahl Tile-Klicks; fixed-Lightbox im perspective-Stacking-Context gefangen → an `<body>`. verify-vacations PASS / check 0 / build grün. | 19.06. |
| C · Camino Blender-Bird's-Eye | ✅ | **19.06.:** paralleler Opus-Agent. `camino_set.py` bäckt kinoreife Bird's-Eye-Kamerafahrt (Catmull-Rom durch 15 echte Waymark-Stationen, „CaminoFlight"-Clip, 73 Quat-Keys + Flip-Guard, 7.0s@30fps) in `camino.glb`. `camino.ts`: Beacons/Cones/Tube + Analytik-Chase raus → AnimationMixer-Scrub (city.ts-Muster). Captions gepurged (Waymark + EOX/Copernicus-Attrib bleiben). **Blender-5.1-Gotcha:** slotted Action hat kein `action.fcurves` → LINEAR via `keyframe_new_interpolation_type` vor Insert. verify-camino PASS / 85-120fps / 0 Errors. „Google-Maps × crazy besser", keine Floating-Marker. | 19.06. |
| D · Das Boot (The Sea) | ✅ | **19.06.:** 2 parallele Opus-Agenten (Schiff #1 + Detail-Rebuild #2). Brig **1.7k→21.6k tris** = echter 1820er-Dreimaster (geplankter Rumpf, Bugspriet+Jibboom+Dolphin-Striker, 3 voll besparte Masten, Rahen+Footropes, volle gewölbte Segel+Jibs+Spanker, **dichte Wanten mit Ratlines pro Panel** = backlit-Lace, Decksdetails, Ensign). **2 echte Bugs gefixt:** (a) Lateral-Drift `(t*speed*10)%span` kreuzte nie (~1.4 von 150-Span/Reise) → ein Hero-Crossing `bx=pathXFrom−cross*span` R→L über Scroll; (b) **meshopt fraß die Größe** (Frame-Gotcha Tag 13: `setScalar` überschreibt KHR-Quant-Dequant-Skala → winzig) → **RAW-GLB shippen** (2.9MB<25MB), scale 1.5. + Silhouette abgedunkelt (`a.rgb*0.32`) = Aivazovsky-Cut-out vor der Sonne. `window.__sea`-Hook (Lenis-Scrub). verify-sea PASS / check 0 / build grün / 0 Errors. **Build:** sea_set.py→raw→`cp` nach public (NICHT meshopt). Ozean unangetastet. | 19.06. |
| E · Text-Purge | ✅ | **19.06.:** paralleler Opus-Agent. Frontispiece-Flavor-Taglines + Voyage/Approach/Closing-Captions in `sea.astro`/`city/index.astro`/`horizon.astro` gestrichen (`<!-- TODO(Mattis): final line -->`), „City of Agents"-Reste → „The City". Echte Bio/Records/Kontakt + Attrib bleiben. (16.06.: dt. Leak + passage-Plakette bereits raus.) check 0 / build grün. **Offen:** verwaiste `.c1/.c2/.c3`-CSS-Regeln (harmlos), camino-Captions (via C erledigt). | 19.06. |
