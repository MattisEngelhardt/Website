...
|---|---|---|---|
| A · Main-Lobby (Nebelmeer-Hub) | 🟧 | **REBUILD nach Mattis-Verriss (17.06.):** TSL-Raymarch-Nebel raus (las als Dunst, 4–17fps) → **echte Blender-Nebelmeer-Geometrie** (`scripts/blender/fog_set.py`→`fog.glb`, unlit vertex-luminance + aerial fade + day-cycle tint): dramatisch billowendes, moody Friedrich-Nebelmeer bei **58fps headed-Probe (~80 nativ 4060)**. **Figur entfernt**, Outcrop vergrößert (First-Person-Repoussoir). **Carved Gold-Frames** (`frame_set.py`→`frame.glb`, gefrästes ogee/cove/bead + baked gilt/AO) statt flacher Box-Bevel, **4× instanziert in gestaffelter Tiefe** (nicht mehr Reihe). 2 parallele Opus-Agenten (Blueprint `REDESIGN-LOBBY.md` + frame_set.py). check 0 / build grün (13 S.) / verify-hub PASS / 0 Errors. **Gotchas:** glTF Blender+Z→+Y (Frame lag flach → `rotation.x=π/2`); meshopt-Quant frisst Größe (Frame als RAW-GLB shippen). **Offen:** Outcrop crude low-poly + Plinths (P1.5) fehlen; Sky flach-orange; kuratierte Stills (P1.6) statt roher Screenshots; Placards an Frame ankern; **Integration A5/A6** (worlds/summit.ts + index.astro `<a>` + passage.ts); ehrliche FPS auf nativem Browser. | 17.06. |
