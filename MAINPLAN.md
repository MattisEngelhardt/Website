...
| D · Das Boot (The Sea) | 🟧 | **19.06.:** paralleler Opus-Agent baute detaillierte Brig (`sea_set.py`, Brig/Skiff, +X-Bug, Multi-Punkt-Buoyancy aus seaHeight/seaSlope). Gebaut + auf Wasser geprüft: **(a) las als ferner Fleck** — Ursache: kaputte Lateral-Formel `(t*speed*10)%span` (nur ~1.4 von 150 Span über ganze Reise → kreuzte nie); gefixt zu linearem Sweep über t + scale 1.45→2.6, pathZ −78→−52. **(b) nur 1719 tris = nicht „extrem detailliert"** (Mattis) → **2. Schiff-Agent läuft** (40–90k tris, dichte Takelage/Ratlines/Segel). **Offen:** detailliertes Modell rebuilden + Platzierung final tunen (vor der Sonne kreuzen) + verify-sea. Wasser/Ozean unangetastet (abgenommen). | 19.06. |
