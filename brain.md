...
- ⏳ **Offen Akt 0 (Doktrin-3D-Kern)**: Der **Blender-Summit-Set** steht noch aus — der Threshold dissolved aktuell auf die BESTEHENDE prozedurale Fog-Szene (`scenes/summit.ts`: Canvas-Wanderer, Noise-Fog). Das ist visuell schon stark, aber die Doktrin verlangt echte Geometrie/Bake. Nächster Schritt: `summit_set.py` (Fels aus PolyHaven `cliff_side`/`rock_face`-Texturen, Wanderer-Relief, Cloud-Cards Nebelmeer, Kamerapfad) nach Vorlage `city_set.py`+`city_bake_export.py`, dann `scenes/summit.ts` auf GLB-Load umstellen (Painterly-Post + Linse + Descent behalten). Dann erst echte Side-by-side gegen das Gemälde.
- ⏳ FPS-Check auf Mattis' Gerät weiterhin offen (Threshold-Zoom = nur CSS-transform, sollte 60fps sein; Kuwahara-Kosten unverändert).
